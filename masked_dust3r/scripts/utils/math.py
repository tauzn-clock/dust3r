...
def calculate_new_params(current_parameters, device):
    ...
    a,b,c,d = best_fit_plane(T)
    R = fit_to_xy_plane(a,b,c,d)
    R = torch.tensor(R).to(device).float()

    homogeneous = roma.RigidUnitQuat(Q, T).normalize().to_homogeneous()
    homogeneous = R @ homogeneous

    quat = roma.rotmat_to_unitquat(homogeneous[:,:3,:3])
    ...
    for i in range(len(current_parameters)):
        new_parameters[i] = torch.cat([quat[i], inv_signed_expm1(homogeneous[i,:3,3])])

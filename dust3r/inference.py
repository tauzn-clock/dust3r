...
@torch.no_grad()
def inference_with_mask(pairs, model, device, masks, kernel, batch_size=8, verbose=True):
    if verbose:
        print(f'>> Inference with model on {len(pairs)} image pairs')
    result = []
    ...
    for i in tqdm.trange(0, len(pairs), batch_size, disable=not verbose):
        res = loss_of_one_batch(collate_with_cat(pairs[i:i+batch_size]), model, None, device)
        res["pred1"]["conf"] = mask_to_conf(res["pred1"]["conf"], masks[res["view1"]["idx"][0]], kernel)
        res["pred2"]["conf"] = mask_to_conf(res["pred2"]["conf"], masks[res["view2"]["idx"][0]], kernel)
        result.append(to_cpu(res))
...
def mask_to_conf(conf,mask,kernel):
    size = kernel.shape[0]
    mask = torch.nn.functional.conv2d(mask.unsqueeze(0).unsqueeze(0), kernel.unsqueeze(0).unsqueeze(0), padding=size//2)
    #mask = mask + 0.01
    mask = mask / mask.max()
